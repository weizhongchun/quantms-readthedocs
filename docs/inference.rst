...
Over the years multiple algorithms have been developed to map the identified peptide list into a final protein list.
...
.. note:: Different software will give you different proteins from the same peptide list. In addition, different
          software in combination with different peptide identification tools can give completely different results (read our previous benchmark on the topic [AUDA2017]_)

quantms provides two major methods for protein inference: **bayesian** and **aggregation** methods, each with corresponding parameters and configurations.
...
To enable protein inference in quantms, provide the following parameter: `--protein_inference_method 'bayesian'`.
...
To compute the aggregation score, the algorithm supports three methods: 'best' (default), 'Product', or
'Sum'. If the user wants to change the method for score calculation, use the option: `--protein_score`.

Additional inference parameters
------------------------------------

Both **bayesian** and **aggregation** methods share multiple parameters to perform protein inference.

1. Top PSMs (`top_PSMs`): By default, the algorithms use only the top PSM to perform protein inference. However,
the user can use more PSMs by changing the parameter, e.g., `top_PSMs 3`.

2. Minimum peptides per protein (`min_peptides_per_protein`): The minimum number of peptides for a protein to be considered
for inference. The default value is `1`, but users can change this via `min_peptides_per_protein`.
...
The protein FDR for both methods is estimated using the concatenated target-decoy database
...
for large proteomics experiments [SAV2015a]_.

.. note:: The “picked” TDS [SAV2015a]_ addresses decoy protein overestimation typically observed for the classic TDS and takes into
...
.. [SAV2015a] Savitski MM, Wilhelm M, Hahne H, Kuster B, Bantscheff M. A Scalable Approach for Protein False Discovery Rate Estimation in Large Proteomic Data Sets. Mol Cell Proteomics. 2015 Sep;14(9):2394-404. doi: 10.1074/mcp.M114.046995. Epub 2015 May 17. PMID: 25987413; PMCID: PMC4563723.
